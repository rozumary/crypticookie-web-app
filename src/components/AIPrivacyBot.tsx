...
export const AIPrivacyBot: React.FC = () => {
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="bg-[#0F1523] border border-slate-800 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 px-3 py-0.5 rounded-full bg-slate-800 text-indigo-300 border border-slate-700 text-[11px] font-mono font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-4 flex flex-col">
          ...
          <div className="bg-[#0F1523] border border-slate-800 rounded-2xl p-5 space-y-3 shadow-sm">
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full bg-[#141C2E] border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 placeholder-slate-500 font-mono focus:outline-none focus:border-indigo-500"
                onKeyDown={(e) => e.key === 'Enter' && handleAuditWebsite()}
              />
              <button
                onClick={handleAuditWebsite}
                disabled={isLoading || !auditDomain.trim()}
                className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs transition-all cursor-pointer shadow-sm"
              >
          ...
          <div className="bg-[#0F1523] border border-slate-800 rounded-2xl p-5 space-y-3 flex-1 flex flex-col justify-between shadow-sm">
            <div className="flex items-center gap-2 text-white font-bold text-xs">
              ...
              {DEFAULT_SUGGESTIONS.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full text-left p-3 rounded-xl bg-[#141C2E] hover:bg-slate-800/80 border border-slate-800 hover:border-slate-700 transition-all cursor-pointer group"
                  >
                    <div className="flex items-start gap-2.5">
                      <Icon className="h-4 w-4 text-indigo-400 mt-0.5 shrink-0 group-hover:scale-110 transition-transform" />
                      <div>
        ...
        <div className="lg:col-span-8 flex flex-col rounded-2xl border border-slate-800 bg-[#0F1523] overflow-hidden h-[620px] max-h-[72vh] shadow-sm">
          {/* Chat Window Top Bar */}
          <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-5 py-3 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono font-semibold text-slate-400 hidden sm:inline">
                Encrypted Session
              </span>
          ...
          {/* Messages Stream */}
          <div className="flex-1 p-4 sm:p-5 space-y-3.5 overflow-y-auto bg-[#0A0E17]">
            {messages.map((m) => {
              ...
              return (
                ...
                >
                  {isBot && (
                    <div className="h-8 w-8 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-indigo-300 shrink-0 mt-0.5">
                      <Bot className="h-4 w-4" />
                    </div>
                  )}
                  ...
                  <div
                    className={`relative max-w-[85%] sm:max-w-[75%] rounded-2xl p-4 text-xs leading-relaxed ${
                      isBot
                        ? 'bg-[#141C2E] border border-slate-800 text-slate-200'
                  ...
                  >
                    ...
                    <div className={`flex items-center justify-between gap-4 mt-2.5 pt-2 border-t text-[10px] font-mono ${
                      isBot ? 'border-slate-800 text-slate-400' : 'border-indigo-400/30 text-indigo-100'
                    }`}>
                      <span>{m.timestamp}</span>
                      {isBot && (
                        <button
                          onClick={() => handleCopy(m.content, m.id)}
                          className="flex items-center gap-1 hover:text-white transition-colors cursor-pointer font-bold text-indigo-300"
                          title="Copy response"
                        >
                          {copiedId === m.id ? (
                  ...
                  {DEFAULT_SUGGESTIONS.map((s, idx) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isLoading}
                        className="text-left p-3 rounded-xl bg-[#141C2E] hover:bg-slate-800/80 border border-slate-800 hover:border-slate-700 transition-all cursor-pointer group flex items-start gap-2.5"
                      >
                        <div className="h-7 w-7 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center shrink-0 text-indigo-400 group-hover:scale-105 transition-transform">
            ...
            {isLoading && (
              <div className="flex items-start gap-3 justify-start animate-pulse">
                <div className="h-8 w-8 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-indigo-300 shrink-0">
                  <Bot className="h-4 w-4 animate-spin text-indigo-400" />
                </div>
                <div className="rounded-2xl bg-[#141C2E] border border-slate-800 p-4 text-xs text-slate-300 font-bold flex items-center gap-2">
          ...
          <div className="border-t border-slate-800 bg-slate-900 p-4">
            ...
            >
              <input
                ...
                className="flex-1 bg-[#0A0E17] border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
              />
              <button
                type="submit"
                disabled={isLoading || !inputMessage.trim()}
                className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
              >
                <span>Send</span>
