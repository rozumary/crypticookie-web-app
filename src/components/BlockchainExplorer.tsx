...
export const BlockchainExplorer: React.FC<BlockchainExplorerProps> = ({ onRefreshData }) => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-[#FFFFFF] border border-[#B78AE8] rounded-3xl p-6 sm:p-8 shadow-md shadow-purple-900/5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-[#3B235C] tracking-tight">Blockchain Explorer</h1>
            <span className="px-2.5 py-0.5 rounded-full bg-[#EDE1FF] text-[#8B4ED8] border border-[#B78AE8] text-[11px] font-mono font-semibold">
              Hybrid PB+P Chain
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-100 hover:bg-rose-200 text-rose-800 border border-rose-300 text-xs font-bold transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[#EDE1FF] hover:bg-[#EDE1FF]/80 text-[#3B235C] border border-[#B78AE8] transition-colors cursor-pointer"
          >
      ...
      {verificationResult && (
        <div
          className={`p-6 sm:p-8 rounded-3xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-md shadow-purple-900/5 ${
            verificationResult.isValid
              ? 'border-[#B78AE8] bg-[#EDE1FF] text-[#3B235C]'
        ...
        >
          <div className="flex items-center gap-3">
            {verificationResult.isValid ? (
              ...
              <ShieldAlert className="h-6 w-6 text-rose-700 shrink-0" />
            )}
            <div>
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-[#3B235C]">
                  {verificationResult.isValid
                    ? 'Chain Integrity: 100% Valid'
                    : `Tampered Block Detected at Index #${verificationResult.brokenBlockIndex}`}
                </span>
                <span className="text-[11px] font-mono px-2.5 py-0.5 rounded-full bg-[#FFFFFF] text-[#8B4ED8] border border-[#B78AE8] font-bold">
          ...
          {!verificationResult.isValid && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-xs font-bold text-white cursor-pointer shrink-0 shadow-sm"
            >
        ...
        {activeLedgerTab === 'public' && (
          <div className="space-y-4">
            {filteredPublicBlocks.length === 0 ? (
              <div className="text-center py-10 bg-[#FCFAFF] rounded-2xl border border-[#CBA3F5] text-[#6B528E] text-xs font-mono">
                  ...
                  <div
                    ...
                    className={`p-5 rounded-2xl border transition-all ${
                      isTampered
                        ...
                        : 'border-[#CBA3F5] bg-[#FCFAFF] hover:border-[#B78AE8] shadow-sm'
                    }`}
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <span className="h-7 w-7 rounded-lg bg-[#EDE1FF] border border-[#B78AE8] text-[#8B4ED8] font-mono text-xs font-bold flex items-center justify-center">
                          #{block.block_index}
                        </span>
                        <span className="font-bold text-[#3B235C] text-sm">
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        <span className="font-mono text-[#6B528E] uppercase text-[11px]">
                          Action: <strong className="text-[#3B235C] font-bold">{block.consent_action}</strong>
                        </span>
                        <span className="text-[#6B528E] text-[11px] font-mono">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                      <div className="p-3 rounded-xl bg-[#FFFFFF] border border-[#CBA3F5]">
                        <div className="text-[10px] text-[#6B528E] flex items-center justify-between mb-1 font-semibold">
                          <span>PREV HASH</span>
                          <button
                            onClick={() => handleCopy(block.prev_hash)}
                            className="text-[#8B4ED8] hover:text-[#3B235C]"
                          >
                            {copiedHash === block.prev_hash ? <Check className="h-3 w-3 text-emerald-600" /> : <Copy className="h-3 w-3" />}
                          </button>
                        </div>
                        <span className="text-[#6B528E] block truncate">{block.prev_hash}</span>
                      </div>
                      ...
                      <div className="p-3 rounded-xl bg-[#FFFFFF] border border-[#CBA3F5]">
                        <div className="text-[10px] text-[#6B528E] flex items-center justify-between mb-1 font-semibold">
                          ...
                          <button
                            ...
                            className="text-[#8B4ED8] hover:text-[#3B235C]"
                          >
                            {copiedHash === block.hash ? <Check className="h-3 w-3 text-emerald-600" /> : <Copy className="h-3 w-3" />}
                          </button>
                        </div>
                        <span className={`block truncate ${isTampered ? 'text-rose-700 font-bold' : 'text-emerald-700 font-bold'}`}>
                          {block.hash}
                        </span>
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            {filteredPrivateBlocks.length === 0 ? (
              ...
              filteredPrivateBlocks.map((block) => (
                <div
                  key={block.id}
                  className="p-5 rounded-2xl border border-[#CBA3F5] bg-[#FCFAFF] hover:border-[#B78AE8] transition-all shadow-sm"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-7 w-7 rounded-lg bg-[#EDE1FF] border border-[#B78AE8] text-[#8B4ED8] font-mono text-xs font-bold flex items-center justify-center">
                    ...
                    <div className="flex items-center gap-3 text-xs font-mono">
                      <span className="text-[#6B528E] text-[11px]">User: <strong className="text-[#8B4ED8] font-bold">{block.user_id}</strong></span>
                      <span className="text-[#6B528E] text-[11px]">
                        {new Date(block.timestamp).toLocaleTimeString()}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                    <div className="p-3 rounded-xl bg-[#FFFFFF] border border-[#CBA3F5]">
                      <span className="text-[10px] text-[#6B528E] block mb-1 font-semibold">PREV HASH</span>
                      <span className="text-[#6B528E] block truncate">{block.prev_hash}</span>
                    </div>

                    <div className="p-3 rounded-xl bg-[#FFFFFF] border border-[#CBA3F5]">
                      <span className="text-[10px] text-[#6B528E] block mb-1 font-semibold">PRIVATE BLOCK HASH</span>
                      <span className="text-[#8B4ED8] block truncate font-bold">{block.hash}</span>
                    </div>
                  </div>
                </div>
              ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#3B235C]/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl border border-[#B78AE8] bg-[#FFFFFF] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[#CBA3F5] pb-3">
              <h3 className="text-base font-bold text-[#3B235C] flex items-center gap-2">
                <Flame className="h-4 w-4 text-rose-600" />
                <span>Simulate Database Tampering</span>
              </h3>
              <button
                onClick={() => setIsTamperingModalOpen(false)}
                className="text-[#6B528E] hover:text-[#3B235C] text-lg font-bold"
              >
                &times;
              </button>
            </div>
            ...
            <p className="text-xs text-[#6B528E]">
              Modifies a row in <code className="text-[#8B4ED8] font-mono font-bold">public_ledger</code> to demonstrate cryptographic detection.
            </p>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-[#3B235C] font-semibold mb-1">Target Block #</label>
                <select
                  value={tamperTargetIndex}
                  onChange={(e) => setTamperTargetIndex(Number(e.target.value))}
                  className="w-full rounded-xl bg-[#FCFAFF] border border-[#CBA3F5] p-2.5 text-[#3B235C] font-mono focus:outline-none focus:border-[#8B4ED8]"
                >
                  {publicBlocks.map((b) => (
              ...
              <div>
                <label className="block text-[#3B235C] font-semibold mb-1">Tampered Domain</label>
                <input
                  type="text"
            ...
            <div className="pt-3 border-t border-[#CBA3F5] flex items-center justify-end gap-2">
              <button
                onClick={() => setIsTamperingModalOpen(false)}
                className="px-4 py-2 rounded-xl bg-[#EDE1FF] hover:bg-[#EDE1FF]/80 text-xs text-[#3B235C] font-semibold border border-[#B78AE8] cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isProcessing}
                className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-xs font-bold text-white cursor-pointer shadow-sm"
              >
                Tamper Data
              </button>
            </div>
