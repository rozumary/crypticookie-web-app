      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full rounded-xl bg-slate-900/90 pl-9 pr-3 py-1.5 text-xs text-slate-200 border border-slate-800 focus:outline-none focus:border-indigo-500 font-mono"
          />
        </div>
      </div>
      ...
      {activeLedgerTab === 'public' && (
        <div className="space-y-3">
          {filteredPublicBlocks.length === 0 ? (
            <div className="text-center py-10 bg-slate-900/40 rounded-2xl border border-slate-800 text-slate-500 text-xs font-mono">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-7 w-7 rounded-lg bg-indigo-600/20 text-indigo-300 font-mono text-xs font-bold flex items-center justify-center">
                        #{block.block_index}
                      </span>
          ...
          {filteredPrivateBlocks.length === 0 ? (
            ...
            filteredPrivateBlocks.map((block) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2.5">
                    <span className="h-7 w-7 rounded-lg bg-purple-600/20 text-purple-300 font-mono text-xs font-bold flex items-center justify-center">
                      P#{block.block_index}
                    </span>
                    <span className="font-semibold text-white text-sm">
                      {block.audit_output}
                  ...
                  <div className="flex items-center gap-3 text-xs font-mono">
                    <span className="text-slate-400 text-[11px]">User: <strong className="text-purple-300">{block.user_id}</strong></span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-slate-500 block mb-1">PREV HASH</span>
                    <span className="text-slate-400 block truncate">{block.prev_hash}</span>
                  </div>

                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800">
                    <span className="text-[10px] text-slate-500 block mb-1">PRIVATE BLOCK HASH</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-[#0f172a] p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-400">
              Modifies a row in <code className="text-slate-300">public_ledger</code> to demonstrate cryptographic detection.
            </p>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 mb-1">Target Block #</label>
                <select
              ...
              <div>
                <label className="block text-slate-400 mb-1">Tampered Domain</label>
                <input
                  type="text"
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-end gap-2">
              ...
              <button
                ...
                disabled={isProcessing}
                className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-xs font-semibold text-white cursor-pointer"
              >
                Tamper Data
              </button>
            </div>
