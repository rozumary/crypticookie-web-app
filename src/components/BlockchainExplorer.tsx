...
export const BlockchainExplorer: React.FC<BlockchainExplorerProps> = ({
  ...
  const [activeLedgerTab, setActiveLedgerTab] = useState<'public' | 'private'>(
    'public'
  );
  ...
  const [tamperedAction, setTamperedAction] = useState<
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-[#0E041E] border border-[#391363] rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-xl">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="px-3 py-1 rounded-full bg-[#180735] text-[#E879F9] border border-[#9333EA]/40 text-[11px] font-mono font-bold">
              Hybrid PB+P Chain
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-rose-950/80 hover:bg-rose-900 text-rose-200 border border-rose-800/60 text-xs font-bold transition-all cursor-pointer shadow-sm"
          >
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[#180735] hover:bg-[#250B4E] text-purple-200 border border-[#9333EA]/40 transition-colors cursor-pointer"
          >
            <RotateCw className="h-4 w-4 text-[#E879F9]" />
          </button>
        </div>
      </div>
      ...
      {verificationResult && (
        <div
          className={`p-6 sm:p-8 rounded-3xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl ${
            verificationResult.isValid
              ? 'border-[#391363] bg-[#0E041E] text-purple-100'
              : 'border-rose-800/80 bg-rose-950/60 text-rose-200'
          }`}
        >
          <div className="flex items-center gap-3">
            {verificationResult.isValid ? (
              <ShieldCheck className="h-7 w-7 text-[#10B981] shrink-0" />
            ) : (
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-white">
                  {verificationResult.isValid
                    ? 'Chain Integrity: 100% Valid & Verified'
                    : `Tampered Block Detected at Index #${verificationResult.brokenBlockIndex}`}
                </span>
                <span className="text-[11px] font-mono px-3 py-0.5 rounded-full bg-[#180735] text-[#E879F9] border border-[#9333EA]/40 font-bold">
                  {verificationResult.totalBlocks} Blocks
                </span>
              </div>
              <p className="text-xs text-[#C084FC]/80 mt-0.5">
                {verificationResult.isValid
                  ? 'All sequential cryptographic SHA-256 block hashes are intact and validated.'
      ...
      <div className="bg-[#0E041E] border border-[#391363] rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        {/* Tabs & Search */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#300E54] pb-4">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                activeLedgerTab === 'public'
                  ...
                  : 'bg-[#180735] text-[#D8B4FE] hover:bg-[#250B4E] border border-[#9333EA]/40'
              }`}
            >
          ...
          <div className="relative w-full sm:w-64">
            <Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-[#E879F9]" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search domain or hash..."
              className="w-full rounded-xl bg-[#130526] pl-9 pr-3 py-1.5 text-xs text-purple-100 border border-[#391363] focus:outline-none focus:border-[#9333EA] font-mono placeholder-purple-400/40"
            />
          </div>
        </div>
        ...
        {activeLedgerTab === 'public' && (
          <div className="space-y-4">
            {filteredPublicBlocks.length === 0 ? (
              <div className="text-center py-10 bg-[#130526] rounded-2xl border border-[#300E54] text-[#C084FC]/60 text-xs font-mono">
                  ...
                  <div
                    ...
                    className={`p-5 rounded-2xl border transition-all ${
                      isTampered
                        ...
                        : 'border-[#300E54] bg-[#130526] hover:border-[#9333EA]/50'
                    }`}
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <span className="h-7 w-7 rounded-lg bg-[#180735] border border-[#9333EA]/40 text-[#E879F9] font-mono text-xs font-bold flex items-center justify-center">
                          #{block.block_index}
                        </span>
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        <span className="font-mono text-[#C084FC]/80 uppercase text-[11px]">
                          Action:{' '}
                          <strong className="text-[#E879F9] font-bold">
                            {block.consent_action}
                          </strong>
                        </span>
                        <span className="text-[#C084FC]/60 text-[11px] font-mono">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                      <div className="p-3 rounded-xl bg-[#0E041E] border border-[#300E54]">
                        <div className="text-[10px] text-[#C084FC]/70 flex items-center justify-between mb-1 font-semibold">
                          <span>PREV HASH</span>
                          <button
                            onClick={() => handleCopy(block.prev_hash)}
                            className="text-[#E879F9] hover:text-white cursor-pointer"
                          >
                        ...
                        <span className="text-[#C084FC]/70 block truncate">
                          {block.prev_hash}
                        </span>
                      </div>
          ...
          <div className="space-y-4">
            {filteredPrivateBlocks.length === 0 ? (
              ...
              filteredPrivateBlocks.map((block) => (
                <div
                  key={block.id}
                  className="p-5 rounded-2xl border border-[#300E54] bg-[#130526] hover:border-[#9333EA]/50 transition-all"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-7 w-7 rounded-lg bg-[#180735] border border-[#9333EA]/40 text-[#E879F9] font-mono text-xs font-bold flex items-center justify-center">
                    ...
                    <div className="flex items-center gap-3 text-xs font-mono">
                      <span className="text-[#C084FC]/80 text-[11px]">
                        User:{' '}
                        <strong className="text-[#E879F9] font-bold">
                          {block.user_id}
                        </strong>
                      </span>
                      <span className="text-[#C084FC]/60 text-[11px]">
                        {new Date(block.timestamp).toLocaleTimeString()}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                    <div className="p-3 rounded-xl bg-[#0E041E] border border-[#300E54]">
                      <span className="text-[10px] text-[#C084FC]/70 block mb-1 font-semibold">
                        PREV HASH
                      </span>
                      <span className="text-[#C084FC]/70 block truncate">
                        {block.prev_hash}
                    ...
                    <div className="p-3 rounded-xl bg-[#0E041E] border border-[#300E54]">
                      ...
                      <span className="text-[#E879F9] block truncate font-bold">
                        {block.hash}
                      </span>
                    </div>
                  </div>
      ...
      {isTamperingModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl border border-[#9333EA]/50 bg-[#0E041E] p-6 space-y-4 text-purple-100 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#300E54] pb-3">
              ...
              <button
                ...
                className="text-[#C084FC]/70 hover:text-white text-lg font-bold cursor-pointer"
              >
                &times;
              </button>
            </div>
            ...
            <p className="text-xs text-[#C084FC]/80">
              Modifies a row in{' '}
              <code className="text-[#E879F9] font-mono font-bold">
                public_ledger
              </code>{' '}
              to demonstrate cryptographic detection.
            </p>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTamperTargetIndex(Number(e.target.value))
                  }
                  className="w-full rounded-xl bg-[#130526] border border-[#391363] p-2.5 text-purple-100 font-mono focus:outline-none focus:border-[#9333EA] cursor-pointer"
                >
                  {publicBlocks.map((b) => (
                    <option
                      key={b.id}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-[#130526] border border-[#391363] p-2.5 text-purple-100 font-mono focus:outline-none focus:border-[#9333EA]"
                />
              </div>
            </div>

            <div className="pt-3 border-t border-[#300E54] flex items-center justify-end gap-2">
              <button
                onClick={() => setIsTamperingModalOpen(false)}
                className="px-4 py-2 rounded-xl bg-[#180735] hover:bg-[#250B4E] text-xs text-purple-200 font-semibold border border-[#9333EA]/40 cursor-pointer"
              >
                Cancel
              </button>
              <button
