        ...
        >
          ...
          {!verificationResult.isValid && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-xs font-semibold text-white cursor-pointer shrink-0"
            >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-medium transition-all cursor-pointer ${
              activeLedgerTab === 'public'
                ? 'bg-purple-600 text-white font-semibold shadow-md shadow-purple-950/60'
                : 'bg-[#180a2b] text-purple-300/70 hover:text-white border border-purple-600/50 hover:border-purple-700/50'
            }`}
          >
      ...
      {activeLedgerTab === 'public' && (
        <div className="space-y-3">
          {filteredPublicBlocks.length === 0 ? (
            <div className="text-center py-10 bg-[#180a2b]/40 rounded-2xl border border-purple-600/50 text-purple-300/60 text-xs font-mono">
                ...
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all ${
                    isTampered
                      ...
                      : 'border-purple-600/50 bg-[#180a2b] hover:border-purple-700/40 shadow-sm'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5">
                      <span className="h-7 w-7 rounded-lg bg-purple-600/20 border border-purple-500/30 text-purple-300 font-mono text-xs font-bold flex items-center justify-center">
                        #{block.block_index}
                      </span>
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      <span className="text-purple-300/50 text-[11px] font-mono">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                    <div className="p-2.5 rounded-xl bg-[#0f051c] border border-purple-950">
                      <div className="text-[10px] text-purple-300/60 flex items-center justify-between mb-1">
                        <span>PREV HASH</span>
                        <button
        ...
        <div className="space-y-3">
          {filteredPrivateBlocks.length === 0 ? (
            ...
            filteredPrivateBlocks.map((block) => (
              <div
                key={block.id}
                className="p-4 rounded-2xl border border-purple-600/50 bg-[#180a2b] hover:border-purple-700/40 transition-all shadow-sm"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2.5">
                    <span className="h-7 w-7 rounded-lg bg-purple-600/20 border border-purple-500/30 text-purple-300 font-mono text-xs font-bold flex items-center justify-center">
                  ...
                  <div className="flex items-center gap-3 text-xs font-mono">
                    <span className="text-purple-300/70 text-[11px]">User: <strong className="text-purple-300">{block.user_id}</strong></span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono">
                  <div className="p-2.5 rounded-xl bg-[#0f051c] border border-purple-950">
                    <span className="text-[10px] text-purple-300/60 block mb-1">PREV HASH</span>
                    <span className="text-purple-300/70 block truncate">{block.prev_hash}</span>
                  </div>

                  <div className="p-2.5 rounded-xl bg-[#0f051c] border border-purple-950">
                    <span className="text-[10px] text-purple-300/60 block mb-1">PRIVATE BLOCK HASH</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-purple-600/50 bg-[#180a2b] p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-purple-200/70">
              Modifies a row in <code className="text-purple-300 font-mono">public_ledger</code> to demonstrate cryptographic detection.
            </p>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-purple-300 mb-1">Target Block #</label>
                <select
                  value={tamperTargetIndex}
                  onChange={(e) => setTamperTargetIndex(Number(e.target.value))}
                  className="w-full rounded-xl bg-[#0f051c] border border-purple-500/50 p-2.5 text-white font-mono focus:outline-none focus:border-purple-500"
                >
                  {publicBlocks.map((b) => (
              ...
              <div>
                <label className="block text-purple-300 mb-1">Tampered Domain</label>
                <input
                  type="text"
            ...
            <div className="pt-2 border-t border-purple-600/50 flex items-center justify-end gap-2">
              <button
                onClick={() => setIsTamperingModalOpen(false)}
                className="px-4 py-2 rounded-xl bg-purple-950/60 hover:bg-purple-900/60 text-xs text-purple-200 border border-purple-800/40 cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isProcessing}
                className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-xs font-semibold text-white cursor-pointer shadow-sm"
              >
                Tamper Data
              </button>
            </div>
