      ...
      <div className="bg-[#0F1523] border border-slate-800 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-indigo-300 text-[10px] font-mono font-bold border border-slate-700 flex items-center gap-1">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-xs font-bold text-white transition-all cursor-pointer shadow-sm"
        >
        ...
        <div className="bg-[#141C2E] border border-slate-800 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-slate-900 text-slate-300 border-b border-slate-800 text-[11px] font-bold">
                <tr>
                  ...
                  filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        ...
                        >
                          <option value="whitelist" className="bg-[#0F1523]">whitelist</option>
                          <option value="blacklist" className="bg-[#0F1523]">blacklist</option>
                        </select>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-[#0F1523] p-6 space-y-4 text-slate-200">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setIsAddModalOpen(false)} className="text-slate-400 hover:text-white text-lg font-bold cursor-pointer">
                &times;
              </button>
            </div>
            ...
            <form onSubmit={handleAddItem} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">CMP / Framework Name</label>
                <input
                  type="text"
                  value={newCmpName}
                  ...
                  required
                  className="w-full rounded-xl bg-[#141C2E] border border-slate-800 p-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 placeholder-slate-500"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">Script URL or Raw JS (Hashed)</label>
                <input
                  type="text"
                  value={newScriptInput}
                  onChange={(e) => handleComputeHash(e.target.value)}
                  placeholder="Paste script content or URL to hash"
                  className="w-full rounded-xl bg-[#141C2E] border border-slate-800 p-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 font-mono placeholder-slate-500"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">SHA-256 Hash</label>
                <input
                  type="text"
                  value={newScriptHash}
                  ...
                  required
                  className="w-full rounded-xl bg-[#141C2E] border border-slate-800 p-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 font-mono text-[11px] placeholder-slate-500"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">Registry Classification</label>
                <select
                  value={newStatus}
                  onChange={(e) => setNewStatus(e.target.value as CMPStatus)}
                  className="w-full rounded-xl bg-[#141C2E] border border-slate-800 p-2.5 text-slate-100 focus:outline-none focus:border-indigo-500 cursor-pointer font-semibold"
                >
                  <option value="whitelist" className="bg-[#0F1523]">Whitelist (Trusted)</option>
                  <option value="blacklist" className="bg-[#0F1523]">Blacklist (Deceptive / Threat)</option>
                </select>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-end gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold cursor-pointer"
                >
                  Cancel
                </button>
                <button
