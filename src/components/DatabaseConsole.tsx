...
export const DatabaseConsole: React.FC<DatabaseConsoleProps> = ({ onRefreshData }) => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#180a2b] hover:bg-purple-950/60 text-purple-200 border border-purple-600/50 text-xs font-medium transition-colors cursor-pointer"
          >
      ...
      <div className="rounded-2xl border border-purple-600/50 bg-[#180a2b] p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs text-purple-200/80 shadow-sm">
        ...
        <div className="bg-[#0f051c] border border-purple-500/50 p-3 rounded-xl text-[11px] font-mono shrink-0 space-y-1.5 min-w-[260px]">
          <div className="flex items-center justify-between gap-4">
            <span className="text-purple-400/70">GCP Project:</span>
            <span className="text-white font-semibold">{firebaseConfigData.projectId}</span>
          </div>
          <div className="flex items-center justify-between gap-4">
            <span className="text-purple-400/70">Database ID:</span>
            <span className="text-emerald-400 font-semibold truncate max-w-[160px]">{firebaseConfigData.firestoreDatabaseId || '(default)'}</span>
          </div>
          <div className="pt-1.5 border-t border-purple-600/50 text-[10px] text-purple-300/50">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className="p-2 rounded-xl bg-[#180a2b] hover:bg-purple-950/60 text-purple-300 border border-purple-600/50 transition-colors cursor-pointer shrink-0"
        >
      ...
      <div className="rounded-2xl border border-purple-600/50 bg-[#180a2b] overflow-hidden shadow-sm">
        <div className="p-3 bg-[#0f051c] border-b border-purple-600/50 flex items-center justify-between text-xs">
          <span className="font-mono text-purple-200">Table: {selectedTable}</span>
          <span className="text-purple-300/60 font-mono">{tableData.length} records</span>
        </div>

        <div className="overflow-x-auto max-h-[480px]">
          {tableData.length === 0 ? (
            <div className="p-8 text-center text-xs text-purple-300/50 font-mono">
              Table is empty.
            </div>
          ) : (
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-[#0f051c] text-purple-300/70 border-b border-purple-600/50 sticky top-0">
                <tr>
              ...
              <tbody className="divide-y divide-purple-900/30">
                {tableData.map((row, i) => (
                  <tr key={row.id || i} className="hover:bg-purple-950/20">
                    {Object.keys(tableData[0] || {}).map((col) => {
                      const val = row[col];
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md rounded-2xl border border-rose-500/30 bg-[#180a2b] p-6 shadow-2xl space-y-4 relative">
            <div className="flex items-center gap-3 border-b border-purple-600/50 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-purple-200/70 mt-0.5">This action will reseed all tables</p>
              </div>
              <button
                onClick={() => setIsResetModalOpen(false)}
                className="absolute right-4 top-4 text-purple-400 hover:text-white p-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="pt-3 border-t border-purple-600/50 flex items-center justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setIsResetModalOpen(false)}
                disabled={isResetting}
