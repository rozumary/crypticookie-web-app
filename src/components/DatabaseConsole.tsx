...
export const DatabaseConsole: React.FC<DatabaseConsoleProps> = ({ onRefreshData }) => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#0b1026] hover:bg-blue-950/60 text-blue-200 border border-blue-900/40 text-xs font-medium transition-colors cursor-pointer"
          >
      ...
      <div className="rounded-2xl border border-blue-900/40 bg-[#0b1026]/90 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs text-blue-200/80 shadow-sm">
        ...
        <div className="bg-[#060a17] border border-blue-900/50 p-3 rounded-xl text-[11px] font-mono shrink-0 space-y-1.5 min-w-[260px]">
          <div className="flex items-center justify-between gap-4">
            <span className="text-blue-400/70">GCP Project:</span>
            <span className="text-white font-semibold">{firebaseConfigData.projectId}</span>
          </div>
          <div className="flex items-center justify-between gap-4">
            <span className="text-blue-400/70">Database ID:</span>
            <span className="text-emerald-400 font-semibold truncate max-w-[160px]">{firebaseConfigData.firestoreDatabaseId || '(default)'}</span>
          </div>
          <div className="pt-1.5 border-t border-blue-900/40 text-[10px] text-blue-300/50">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className="p-2 rounded-xl bg-[#0b1026] hover:bg-blue-950/60 text-blue-300 border border-blue-900/40 transition-colors cursor-pointer shrink-0"
        >
      ...
      <div className="rounded-2xl border border-blue-900/40 bg-[#0b1026]/90 overflow-hidden shadow-sm">
        <div className="p-3 bg-[#060a17] border-b border-blue-900/40 flex items-center justify-between text-xs">
          <span className="font-mono text-blue-200">Table: {selectedTable}</span>
          <span className="text-blue-300/60 font-mono">{tableData.length} records</span>
        </div>

        <div className="overflow-x-auto max-h-[480px]">
          {tableData.length === 0 ? (
            <div className="p-8 text-center text-xs text-blue-300/50 font-mono">
              Table is empty.
            </div>
          ) : (
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-[#060a17] text-blue-300/70 border-b border-blue-900/40 sticky top-0">
                <tr>
              ...
              <tbody className="divide-y divide-blue-900/30">
                {tableData.map((row, i) => (
                  <tr key={row.id || i} className="hover:bg-violet-950/20">
                    {Object.keys(tableData[0] || {}).map((col) => {
                      const val = row[col];
                      ...
                      return (
                        <td key={col} className="py-2.5 px-4 text-blue-100 max-w-xs truncate">
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md rounded-2xl border border-rose-500/30 bg-[#0b1026] p-6 shadow-2xl space-y-4 relative">
            <div className="flex items-center gap-3 border-b border-blue-900/40 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-200/70 mt-0.5">This action will reseed all tables</p>
              </div>
              <button
                onClick={() => setIsResetModalOpen(false)}
                className="absolute right-4 top-4 text-blue-400 hover:text-white p-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs text-blue-100">
              ...
              <ul className="list-disc list-inside space-y-1 text-blue-200/80 pl-1 font-mono text-[11px]">
            ...
            <div className="pt-3 border-t border-blue-900/40 flex items-center justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setIsResetModalOpen(false)}
                disabled={isResetting}
