...
export const DatabaseConsole: React.FC<DatabaseConsoleProps> = ({ onRefreshData }) => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-gradient-to-r from-[#2e124f]/90 via-[#250d42]/90 to-[#2e124f]/90 border border-purple-400/30 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-xl shadow-purple-950/20 backdrop-blur-md">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-[#371661] text-pink-300 text-[11px] font-mono font-bold border border-purple-400/40 flex items-center gap-1 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-[#371661] hover:bg-[#471c7d] text-purple-200 border border-purple-400/40 text-xs font-bold transition-colors cursor-pointer shadow-sm"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-rose-950/70 hover:bg-rose-900/80 text-rose-200 border border-rose-700/50 text-xs font-bold transition-colors cursor-pointer shadow-sm"
          >
      ...
      <div className="bg-gradient-to-r from-[#2a104a]/90 to-[#200b3b]/90 border border-purple-400/30 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs text-purple-100 shadow-xl shadow-purple-950/20 backdrop-blur-md">
        <div className="flex items-start gap-3">
          <div className="h-10 w-10 rounded-2xl bg-[#371661] border border-purple-400/40 flex items-center justify-center text-pink-300 shrink-0 mt-0.5 shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="px-2 py-0.5 rounded-full bg-[#371661] text-pink-300 border border-purple-400/40 text-[10px] font-mono font-bold">
        ...
        <div className="bg-[#1e0a36] border border-purple-400/35 p-3.5 rounded-2xl text-[11px] font-mono shrink-0 space-y-1.5 min-w-[260px] shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <span className="text-purple-300/80 font-bold">GCP Project:</span>
            <span className="text-white font-bold">{firebaseConfigData.projectId}</span>
          </div>
          <div className="flex items-center justify-between gap-4">
            <span className="text-purple-300/80 font-bold">Database ID:</span>
            <span className="text-emerald-400 font-bold truncate max-w-[160px]">{firebaseConfigData.firestoreDatabaseId || '(default)'}</span>
          </div>
          <div className="pt-1.5 border-t border-purple-400/25 text-[10px] text-purple-300/60">
      ...
      <div className="bg-gradient-to-b from-[#2a104a]/90 to-[#200b3b]/90 border border-purple-400/30 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl shadow-purple-950/20 backdrop-blur-md">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            {[
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-mono transition-all cursor-pointer whitespace-nowrap shadow-sm ${
                  selectedTable === tab.id
                    ? 'bg-gradient-to-r from-pink-600 to-purple-600 text-white font-bold'
                    : 'bg-[#1e0a36] text-purple-200 hover:bg-[#371661] border border-purple-400/35'
                }`}
              >
                {tab.label}
              </button>
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[#371661] hover:bg-[#471c7d] text-purple-200 border border-purple-400/40 transition-colors cursor-pointer shrink-0 shadow-sm"
          >
        ...
        {/* Table Content */}
        <div className="rounded-2xl border border-purple-400/30 bg-[#1e0a36] overflow-hidden shadow-md">
          <div className="p-3 bg-[#2c114d] border-b border-purple-400/30 flex items-center justify-between text-xs font-bold text-white">
          ...
          <div className="overflow-x-auto max-h-[480px]">
            {tableData.length === 0 ? (
              <div className="p-8 text-center text-xs text-purple-300/70 font-mono">
                Table is empty.
              </div>
            ) : (
              <table className="w-full text-left text-xs font-mono">
                <thead className="bg-[#2c114d] text-purple-200 border-b border-purple-400/30 sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-purple-500/20 bg-[#1e0a36]">
                  {tableData.map((row, i) => (
                    <tr key={row.id || i} className="hover:bg-[#2e134f]/70 transition-colors">
                      {Object.keys(tableData[0] || {}).map((col) => {
                        const val = row[col];
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md rounded-3xl border border-rose-500/50 bg-gradient-to-b from-[#2e124f] to-[#1e0a36] p-6 space-y-4 relative text-purple-100 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-purple-400/30 pb-3">
              <div className="h-10 w-10 rounded-2xl bg-rose-950/80 border border-rose-500/50 text-rose-300 flex items-center justify-center shrink-0">
                <AlertTriangle className="h-5 w-5" />
              </div>
              <div>
                <h3 className="text-base font-bold text-white">Reset Database to Default?</h3>
                <p className="text-xs text-purple-200/80 mt-0.5">This action will reseed all tables</p>
              </div>
              <button
                onClick={() => setIsResetModalOpen(false)}
                className="absolute right-4 top-4 text-purple-300 hover:text-white p-1 cursor-pointer"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs text-purple-100">
              ...
              <ul className="list-disc list-inside space-y-1 text-purple-200/80 pl-1 font-mono text-[11px]">
            ...
            <div className="pt-3 border-t border-purple-400/30 flex items-center justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setIsResetModalOpen(false)}
                disabled={isResetting}
                className="px-4 py-2 rounded-xl bg-[#371661] hover:bg-[#471c7d] text-xs font-bold text-purple-200 border border-purple-400/40 cursor-pointer"
              >
                Cancel
              </button>
              <button
