...
export const InstallExtensionModal: React.FC<InstallExtensionModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4 animate-fadeIn">
      <div className="w-full max-w-2xl rounded-3xl border border-violet-500/40 bg-[#070b19] p-6 sm:p-8 shadow-2xl shadow-violet-950/80 space-y-6 relative overflow-hidden">
        {/* Glow effect */}
        ...
        {/* Modal Header */}
        <div className="flex items-start justify-between relative z-10 border-b border-blue-900/40 pb-4">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-2xl bg-gradient-to-tr from-violet-600 to-blue-600 p-0.5 shadow-lg shadow-violet-600/30 shrink-0 flex items-center justify-center">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl text-blue-300/60 hover:text-white hover:bg-blue-900/40 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="relative z-10 p-5 rounded-2xl bg-gradient-to-r from-violet-950/40 to-blue-950/40 border border-violet-500/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-lg">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-sm font-bold text-white">
              <FolderArchive className="h-4 w-4 text-violet-400" />
              <span>Step 1: Download Extension Package (.zip)</span>
            </div>
            <p className="text-xs text-blue-200/80">
              Generates complete Chromium extension bundle (<code className="text-violet-300">manifest.json</code>, <code className="text-violet-300">background.js</code>, <code className="text-violet-300">content.js</code>, popup UI).
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 text-white text-xs font-bold shadow-lg shadow-violet-900/50 transition-all cursor-pointer shrink-0 disabled:opacity-50"
          >
            {isDownloading ? (
              <>
        ...
        <div className="relative z-10 space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-blue-300/70 font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div
              ...
              className={`p-3.5 rounded-2xl border transition-all cursor-pointer ${
                activeStep === 1
                  ? 'bg-violet-950/40 border-violet-500/60 shadow-md shadow-violet-950/40'
                  : 'bg-[#0b1026] border-blue-900/40 opacity-80 hover:opacity-100'
              }`}
            >
              <div className="flex items-center gap-2 font-bold text-xs text-white">
                <span className="h-5 w-5 rounded-full bg-violet-600 flex items-center justify-center text-[10px] text-white font-mono">1</span>
                <span>Extract the ZIP</span>
              </div>
              <p className="text-[11px] text-blue-200/70 mt-1.5 leading-relaxed">
                Right-click the downloaded <code className="text-violet-300">crypticookie-manifest-v3-extension.zip</code> and select <strong>"Extract All"</strong> to a folder.
              </p>
            </div>
        ...
        <div className="relative z-10 p-3.5 rounded-xl bg-[#0b1026] border border-blue-900/40 flex items-center justify-between text-xs text-blue-200/80">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="px-3 py-1 rounded-lg bg-blue-950/60 hover:bg-blue-900/60 text-white font-medium text-xs cursor-pointer border border-blue-800/40"
          >
            Got It
          </button>
        </div>
